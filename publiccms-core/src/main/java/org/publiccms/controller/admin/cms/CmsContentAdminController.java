...
public class CmsContentAdminController extends AbstractController {
	...
	public String save(CmsContent entity, CmsContentAttribute attribute, @ModelAttribute CmsContentParamters contentParamters,
		...
		if (notEmpty(author) && empty(entity.getAuthor())){
		...
		if (null != attribute.getText()) {
			...
			String cover = matchingsrc(text);
			if (notEmpty(cover) && empty(entity.getCover())){
				entity.setCover(cover);
			}
			String description = Html2Text(text);       
			if (notEmpty(description) && empty(entity.getDescription())){
		...
		templateComponent.createContentMobileFile(site, entity, category, categoryModel);// 静态化
		if (null != checked && checked) {
			...
			templateComponent.createCategoryMobileFile(site, category, null, null);
		}
		return TEMPLATE_DONE;
	}

	public String matchingsrc(String str){
		...
		if(m.find()) {
			list.add(m.group(1)) ;
			textStr = list.get(0).replaceAll("'", "");
			if(textStr.indexOf("http")==-1) {
				textStr = textStr.substring(textStr.indexOf("/a/")+3, textStr.length());
			} else {
				textStr = "";	
			}
	...
	public String Html2Text(String inputString){
		...
		textStr = StringUtils.substring(htmlStr, 0,100);
		textStr = StringUtils.substringBefore(textStr, "。")+"。";//一直找到最后一个指定的字符串
	...
	public String publish(Long[] ids, HttpServletRequest request, HttpSession session, ModelMap model) {
		...
		if (notEmpty(ids)) {
			for (CmsContent entity : service.getEntitys(ids)) {
				...
				if (null != categoryModel && !entity.isOnlyUrl()
						&& !templateComponent.createContentMobileFile(site, entity, null, categoryModel)) {
					verifyCustom("static", true, model);
					//return TEMPLATE_ERROR;
				}
			}
