 ...
 */
@Component
public class CmsRecycleContentListDirective extends AbstractTemplateDirective {
    ...
    public void execute(RenderHandler handler) throws IOException, Exception {
        ...
        if (handler.getBoolean("advanced", true)) {
            status = handler.getIntegerArray("status");
            disabled = handler.getBoolean("disabled", true);
        ...
        PageHandler page = service.getPage(getSite(handler).getId(), status, handler.getInteger("categoryId"),
                ...
                handler.getString("orderField"), handler.getString("orderType"), null, handler.getInteger("pageIndex", 1),
