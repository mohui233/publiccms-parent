...
public class CmsContentDao extends BaseDao<CmsContent> {
	...
	public PageHandler getPage(Integer siteId, Integer[] status, Integer categoryId, Integer[] categoryIds, Boolean disabled,
			...
			String orderType, Boolean queue, Integer pageIndex, Integer pageSize) {
	 ...
	 * @return
	 */
	public PageHandler getStatePage(Long id, int sort,Integer siteId, Integer[] status, Integer categoryId, Integer[] categoryIds, Boolean disabled,
			String[] modelIds, Long parentId, Boolean emptyParent, Boolean onlyUrl, Boolean hasImages, Boolean hasFiles,
			String title, Long userId, Long checkUserId, Date startPublishDate, Date endPublishDate, String orderField,
			String orderType, Boolean queue, String pageState, Integer pageIndex, Integer pageSize) {
		QueryHandler queryHandler = getQueryHandler("from CmsContent bean");
		if (notEmpty(siteId)) {
			queryHandler.condition("bean.siteId = :siteId").setParameter("siteId", siteId);
		}
		...
		if (pageState.equals("next")) {
			queryHandler.condition("( bean.sort < :sort  OR ( bean.sort = :sort AND ( bean.publishDate < :startPublishDate OR ( bean.publishDate = :startPublishDate AND bean.id < :id))))").setParameter("sort", sort)
			.setParameter("startPublishDate", startPublishDate)
			.setParameter("id", id);
			queryHandler.order("bean.sort desc");
			queryHandler.order("bean.publishDate desc");
			queryHandler.order("bean.id desc");
			...
			queryHandler.order("bean.publishDate asc");
	 ...
	 * @return
	 */
	public PageHandler getRelatedPage(Long id, Integer siteId, Integer[] status, Integer categoryId, Integer[] categoryIds, Boolean disabled,
			String[] modelIds, Long parentId, Boolean emptyParent, Boolean onlyUrl, Boolean hasImages, Boolean hasFiles,
			String title, Long userId, Long checkUserId, Date startPublishDate, Date endPublishDate, String orderField,
